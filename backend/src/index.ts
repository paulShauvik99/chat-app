...
type Friend = {
    ...
    friendId : string,
    roomId? : string,
    status : "ACCEPTED" | "PENDING_REQUEST" | "REJECTED"
}
...
async function createUsers({email, name , password , phone } : User){
...
async function addFriend({userId , friendId, roomId , status} : Friend){
...
async function createGroup({ userId : userIds , groupName : grpname , roomId : roomId }: Group){
...
async function startChat({ userIds : userIds , senderId : senderId , roomId : roomId , type : type, content : content} : Message){
...
async function updateChatMessage({ chatId : chatId, senderId : senderId, content : content } : UpdateChat){
